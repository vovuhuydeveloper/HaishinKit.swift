...
public struct SRTSocketOption: Sendable {
    static func from(uri: URL?) -> [SRTSocketOption] {
        guard let uri else {
            return []
        }
    ...
    static func configure(_ socket: SRTSOCKET, restriction: Restriction, options: [SRTSocketOption]) -> [String] {
        var failures: [String] = []
    ...
    private static func getQueryItems(uri: URL) -> [String: String] {
